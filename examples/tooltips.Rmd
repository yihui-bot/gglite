...
`crosshairsStrokeOpacity` to customize crosshair appearance. G2 defaults to
`strokeOpacity = 0.5` (semi-transparent), which can make custom colors appear
washed-out on a white background. Set `crosshairsStrokeOpacity = 1` for a
fully opaque line.

```{r}
p |> tooltip_(
  crosshairs = TRUE, crosshairsStroke = 'tomato',
  crosshairsLineWidth = 2, crosshairsStrokeOpacity = 1
)
```
...
clearly visible. The vertical crosshair that appears on hover is the default
behavior of `mark_line()` and is not inherited from previous charts.
